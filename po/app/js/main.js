...
//全域
var ui,at,gi,gn,gd,ga,gm,device_token,zoom_out_cnt,zoom_in_cnt,filter_name,
default_group,group_id,group_name,timeline_id,post_tmp_url,activityTimeout;
    ...
    result.complete(function(data){
        if(data.status != 200){
            ...
            ajaxDo(api_name,headers,method,true).complete(function(data){
                ...
                $(".sm-user-area-r, .main-user-area-r").html("秀賢<br/>大家好");
            });
...
//文章區塊 編輯按鈕
$(document).on('click','.st-sub-box-more-btn',function(){
	//按下效果
...
$(document).on('click','.st-more-close',function(){
	zoom_in_cnt = 100;
	zoomIn($(this).parent());
});
...
$('.ap-textarea-desc').keyup(function(){
...
$(".ap-post").click(function(){
   ...
   if(!ap_title){
	   ...
	   var api_name = "groups/"+ group_id +"/timelines/"+ timeline_id +"/events";
       var headers = {
               "ui":ui,
               "at":at, 
               "li":"zh_TW"
...
function timelineListWrite(){
    ...
    result.complete(function(data){
        ...
        $(".st-feedbox-area2").html('');
        $.each(timeline_list,function(i,val){
        	var display = "none";
        	var url_chk = val.ml[0].c.split(' ');
        	box_content = val.ml[0].c;
        	if(url_chk[0].substring(0, 7) == 'http://' || url_chk[0].substring(0, 8) == 'https://'){
        		//youtube
            	if(val.ml[0].c.match(/youtube.com|youtu.be/)){
            		var strpos,result={};
            		if(url_chk[0].match(/youtube.com/)){
            			strpos = url_chk[0].indexOf("?v=")+3;
            		}else{
            			strpos = url_chk[0].indexOf("youtu.be")+9;
            		}
            		youtube_code = url_chk[0].substring(strpos,strpos+11);
            		box_content = box_content + '<br/><br/><div class="center-style"><iframe width="320" height="280" src="//www.youtube.com/embed/'+ youtube_code +'" frameborder="0" allowfullscreen></iframe></div>';
            	}else{
            		var q = 'http://query.yahooapis.com/v1/public/yql?q=' + encodeURIComponent('select * from html where url="' + url_chk[0] + '" and xpath="//img|//head/meta|//title"') + '&format=json&callback=?';
					$.ajax({
						type: 'GET',
						url: q, 
						dataType: 'jsonp',
						success: function(data, textStatus) {
							...
							$.each(data.query.results.meta, function(key, val){
							    ...
							    // 取圖片
							    if (!result.img && val.content.substring(0, 7) == 'http://' && val.content.match(/\.jpg|\.png/)) {
							...
							console.log(result);
							//如果meta圖片存在 沒圖的話就從網頁裡的img tag裡面隨便找一張
							if(!result.img && data.query.results.img){
							...
							console.log(url_chk[0]);
							if(result.img && result.title){
								prelink_pic = "<img src='" + result.img + "'/>";
								prelink_title = result.title
								prelink_desc = result.description;
								$('.st-sub-box-2[data-st-cnt="'+i+'"]').append(
										'<div class="st-prelink-div"></div>' +
										'<div class="st-prelink">' +
			                    		'<div class="st-prelink-pic">'+ prelink_pic +'</div>' +
			                    		'<div class="st-prelink-title"><a href="'+ url_chk[0] +'">'+ prelink_title +'</a></div>' +
			                    		'<div class="st-prelink-desc">'+ prelink_desc +'</div>' +
			                    		'</div>'
								);
							}
					      }
            ...
            content = 
                '<div class="st-sub-box">' +
                    '<img class="st-sub-box-more-btn" src="images/timeline/timeline_feedbox_icon_list.png"/>' +
                    '<div class="st-sub-box-more">' +
                       '<div class="st-more-close"></div>' +
                       '<div data-st-more="top" class="st-sub-box-more-box"><img src="images/timeline/timeline_hiddenmenu_icon_top.png"/><div>置頂</div></div>' +
                       '<div data-st-more="subscribe" class="st-sub-box-more-box"><img src="images/timeline/timeline_hiddenmenu_icon_subscribe.png"/><div>訂閱</div></div>' +
                       '<div data-st-more="copy" class="st-sub-box-more-box"><img src="images/timeline/timeline_hiddenmenu_icon_copy.png"/><div>複製</div></div>' +
                       '<div data-st-more="copyword" class="st-sub-box-more-box"><img src="images/timeline/timeline_hiddenmenu_icon_copyword.png"/><div>複製內容</div></div>' +
                       '<div data-st-more="calendar" class="st-sub-box-more-box"><img src="images/timeline/timeline_hiddenmenu_icon_calendar.png"/><div>加入行事曆</div></div>' +
                       '<div data-st-more="del" class="st-sub-box-more-box"><img src="images/timeline/timeline_hiddenmenu_icon_del.png"/><div>刪除</div></div>' +
                    '</div>' +
                    '<div class="st-sub-box-1">' +
                        '<div class="st-sub-box-1-main">' +
                           '<div class="st-user-pic"><img src="images/user_pic.png"/></div>' +
                           '<div class="st-sub-name">秀賢</div>' +
                           '<div class="st-sub-time"><img src="images/icon/icon_time_gray.png"/>3/26 星期四 10:20</div>' +
                        '</div>' +
                        '<div class="st-sub-box-1-footer">' +
                           '<img src="images/icon/icon_object.png"/>' +
                           '所有人 ' +
                        '</div>' +
                    '</div>' +
                    '<div class="st-sub-box-2" data-st-cnt="'+i+'">' +
                    	box_content +
                    '</div>' +
                    '<div class="st-sub-box-3">' +
                        '<img src="images/timeline/timeline_feedbox_icon_like.png"/><div>0</div>' +
                        '<img src="images/timeline/timeline_feedbox_icon_chat.png"/><div>0</div>' +
                        '<img src="images/timeline/timeline_feedbox_icon_read.png"/><div>0</div>' +
                    '</div>' +
                    '<div class="st-sub-box-4">' +
                        '<div class="st-like-btn">讚</div>' +
                        '<div class="st-message">留言</div>' +
                    '</div>' +
                '</div>';
            $(".st-feedbox-area2").append(content);
