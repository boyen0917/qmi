	...
	timelineSwitch = function (act,reset){
		...
		switch (act) {
	        case "feeds":
				...
				// $(".st-filter-main span").html( $(".st-filter-area > .st-filter-action[data-status='all'] span").html() );
				//filter all
 ...
 */  
	...
	timelineListWrite = function (ct_timer,is_top){
    	...
    	if(!ct_timer && !is_top){
    		...
    		// if(selector.find(".load-chk").length) return false;
    	}
        	...
        	if(data.status == 200){
        		...
        		if(!target_obj.reply){
                    ...
                    if(this_event.find(".st-reply-like-area").is(":visible") && parti_like_arr){
                        cns.debug( "-------------", parti_like_arr.length, "-------------" );
                        if(est){
                            //----------- TODO -----------
                            // 最後按讚的應該在最前面
                            // 等柏言改好後把push改成unshift
                            //----------- TODO -----------
                            ...
                            $.each(parti_like_arr,function(i,val){
                                if(val){
                                    ...
                                    cns.debug("[!]error:", val, i, parti_like_arr.length);
                                }
                            });
                        }
                    } 
	...
	replyReload = function(this_event){
        ...
        getEventDetail(this_event.data("event-id")).complete(function(data){
            if(data.status == 200){
                ...
                // if(!this_event.find(".st-box2-more-desc-detail").is(":visible"))
