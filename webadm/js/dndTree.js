...
function treeMake(treeData,queue,undo_cnt) {
    ...
    // size of the diagram
    var bottom=500;//$('#openImportBtn').position().top + $('#openImportBtn').height() + $('#openImportBtn').offset().top;
    var viewerWidth = $("#tree-container").width();//$(document).width();
...
//        }
    ...
    function update(source) {
        ...
        nodeEnter.append("svg:image")
	        .attr("xlink:href", "images/del.png")
        ...
        node.select('text')
            .attr("x", function(d) {
                return d.children || d._children ? -10 : 90;
            })
        ...
        node.select("rect.nodeCircle")
            .style("fill", function(d) {
                return d._children ? "lightsteelblue" : "#d2d2d2";
            });
        ...
        var nodeExit = node.exit().transition()
            .duration(duration)
            .attr("transform", function(d) {
                return "translate(" + (source.y+40) + "," + (source.x+40) + ")";
            })
            .remove();
        ...
        if(undo_chk || queue.length == 0){
        	undo_chk = false;
        	$("#undo-cnt").val($("#undo-cnt").val()*1+1);
        	
        	queue.splice($("#undo-cnt").val(),queue.length);
        	
        	var obj= structuredClone(root);
        	queue.push(obj);
            console.log(queue);
        }
