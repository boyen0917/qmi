...
// Set up the Detail View animations


clickToDetail = function(detailView) {



	zoomInDetail = function(detailView) {

		detailView.transitioning = true
		
		detailView.originalSuperView = detailView.superView
		detailView.originalFrame = detailView.frame
		detailView.originalScreenFrame = detailView.screenFrame()
		
		PSD.FeedScroll.scrollVertical = false
		
		detailView.superView = null
		detailView.frame = detailView.originalScreenFrame

		animation1 = detailView.animate({
			properties: {scale:1.04, y:(PSD.Screen.height - detailView.height) / 2},
			curve: animationCurve
		})
		...
		animation2.on("end", function() {
			detailView.transitioning = false
		})
	}

	zoomOutDetail = function(detailView) {

		detailView.transitioning = true

		animation1 = detailView.animate({
			properties: {scale:1, y:detailView.originalScreenFrame.y},
			curve: animationCurve
		})
		...
		animation1.on("end", function() {
			detailView.superView = detailView.originalSuperView
			detailView.frame = detailView.originalFrame
